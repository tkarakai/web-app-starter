...
function VerificationEmailHtmlPreview({
  html,
  className,
}: {
  html: string;
  ...
  const rendered = renderVerificationPreview(html);
  return (
    <iframe
      srcDoc={rendered}
      sandbox=""
      title="Email preview"
      className={cn("h-[500px] w-full rounded-md border bg-white", className)}
    />
  );
}
...
type EmailVerificationTemplateEditorProps = {
  onDirtyChange?: (dirty: boolean) => void;
  disabled?: boolean;
  embedded?: boolean;
};

export function EmailVerificationTemplateEditor({
  onDirtyChange,
  disabled = false,
  embedded = false,
  ...
  const handleSave = async () => {
    ...
    if (!draftHtml.includes("{{verification_link}}")) {
      toast.error("HTML template must include {{verification_link}}");
      return;
    }
    if (!draftText.includes("{{verification_link}}")) {
      toast.error("Plain text template must include {{verification_link}}");
      return;
    }
    ...
    try {
      ...
      toast.success("Verification email template saved");
      setMode("view");
    } catch (err) {
      toast.error(
  ...
  const handleReset = async () => {
    ...
    setSaving(true);
    try {
      await removeSetting({ key: "emailVerificationTemplate" });
      toast.success("Verification email template reset to default");
      setMode("view");
    } catch (err) {
      toast.error(
          ...
          <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                This email is sent when users need to verify their email address.
              </p>
              {disabled && (
                <p className="text-xs text-muted-foreground">
                  Enable Email Verification to edit this template.
                </p>
              )}
            ...
            <CardContent className="space-y-5">
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm font-medium">
                  {renderVerificationPreview(templateData.subject)}
                </p>
              </div>
                    ...
                    >
                      <div className="h-full overflow-y-auto rounded-md border bg-muted/30 p-4">
                        <pre className="whitespace-pre-wrap text-[13px] leading-relaxed text-foreground/80">
                          {renderVerificationPreview(templateData.text)}
                        </pre>
                      </div>
                    </div>
                  </div>
        ...
        <AlertDialog open={confirmReset} onOpenChange={setConfirmReset}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This will discard your custom verification email template and
    ...
    <Card className={cardClassName}>
      <CardHeader className="space-y-4">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Customize the verification email. Use the variable below to insert
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {VERIFICATION_TEMPLATE_VARIABLES.map((v) => (
              <VariableBadge
                key={v.name}
                name={v.name}
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="verif-tpl-subject" className="text-sm font-medium">
            Subject line
          </Label>
          <Input
            id="verif-tpl-subject"
            value={draftSubject}
        ...
        <div className="space-y-3">
          ...
          <div className="relative mt-2 h-[500px] overflow-hidden rounded-md">
            ...
            >
              {htmlMode === "edit" ? (
                ...
                <VerificationEmailHtmlPreview html={draftHtml} className="h-full" />
              ) : (
          ...
          {activeBodyTab === "html" &&
            draftHtml &&
            !draftHtml.includes("{{verification_link}}") && (
              <p className="text-xs text-destructive">
                Template must contain {"{{verification_link}}"}
              </p>
          )}
          {activeBodyTab === "text" &&
            draftText &&
            !draftText.includes("{{verification_link}}") && (
              <p className="text-xs text-destructive">
                Template must contain {"{{verification_link}}"}
              </p>
          )}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              verification email template. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
      ...
      <AlertDialog open={confirmCancel} onOpenChange={setConfirmCancel}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You have unsaved changes to the verification email template. Are
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Discard unsaved changes?</AlertDialogTitle>
            <AlertDialogDescription>
              You have unsaved changes to the verification email template.
