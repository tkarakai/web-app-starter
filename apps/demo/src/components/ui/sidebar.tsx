...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn("h-7 w-7 [&_svg]:size-4 [&_svg]:shrink-0 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
  ...
  (
    ...
    const handlePointerMove = React.useCallback(
      (event: React.PointerEvent<HTMLButtonElement>) => {
        ...
        const signedDelta =
          (event.clientX - activeResize.startX) * (activeResize.side === "left" ? 1 : -1)
