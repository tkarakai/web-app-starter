...
cd "$PROJECT_DIR"
command -v node >/dev/null || { echo "Node.js is required for checkout-owned process management." >&2; exit 1; }
...
if [ -f "$PID_FILE" ]; then
    ...
    while IFS= read -r line; do
        ...
        if [ -n "$pid" ] && node "$PROCESS_HELPER" running "$name" "$pid"; then
...
terminate_pid_with_timeout() {
    local pid="$1"
    node "$PROCESS_HELPER" stop --name convex
...
if [ "$NEED_CONVEX" = true ]; then
    ...
    node "$PROCESS_HELPER" track convex "$CONVEX_PID"
...
start_next_app() {
    ...
    local next_pid=$!
    node "$PROCESS_HELPER" track "next-${app_name}" "$next_pid"
