...
export function readRecords(root: string): Records {
  ...
  const data = JSON.parse(fs.readFileSync(file, "utf8")) as { root?: unknown; services?: unknown };
...
export function writeRecords(root: string, records: Records): void {
  const temp = path.join(root, `.dev-processes-${process.pid}.tmp`);
  fs.writeFileSync(temp, JSON.stringify({ root, services: records }));
...
export function stop(root: string, name?: string): void {
  ...
  if (fs.existsSync(legacy)) {
    const stopped = new Set(selected.map(([service]) => service));
    ...
    if (remaining.length) fs.writeFileSync(legacy, remaining.join("\n") + "\n");
