...
function fixture(t: TestContext) {
  ...
  for (const name of [
    ...
    "app.config.ts",
    "packages/app-config/src/schema.ts",
    "package.json",
  ]) {
  ...
  const run = () => spawnSync("bash", [join(root, "scripts/copy-shared-assets.sh")], { encoding: "utf8", timeout: 10_000 });
...
test("an app-owned icon named in app.config.ts replaces the starter's", (t) => {
  ...
  f.configure('svg: "packages/design-system/assets/icon.svg"', 'svg: "branding/acme.svg"');
  const result = f.run();
  assert.equal(result.status, 0, result.stderr);
  for (const app of apps) {
    assert.equal(f.read(`apps/${app}/public/icon.svg`), "acme icon");
    assert.equal(f.read(`apps/${app}/public/favicon.ico`), "shared:favicon.ico");
  }
});
...
test("a missing icon source fails before any app output changes", (t) => {
  ...
  f.configure('svg: "packages/design-system/assets/icon.svg"', 'svg: "branding/missing.svg"');
  const result = f.run();
...
test("an invalid app.config.ts stops the copy", (t) => {
  const f = fixture(t);
  f.configure('svg: "packages/design-system/assets/icon.svg"', 'svg: "../outside.svg"');
  const result = f.run();
  assert.notEqual(result.status, 0);
