...
function SidebarProvider({
  ...
  const setSidebarWidth = React.useCallback((nextWidth: number) => {
    const width = Math.round(clampSidebarWidth(nextWidth) * 100) / 100;
  ...
  React.useEffect(() => {
    const cookieValue = Number.parseFloat(getCookieValue(SIDEBAR_WIDTH_COOKIE_NAME) ?? "");
    if (Number.isFinite(cookieValue)) {
      setSidebarWidthState(clampSidebarWidth(cookieValue));
    }
  }, []);
...
function SidebarTrigger({
  ...
  return (
    <button
      ...
      className={cn(
        "h-7 w-7 [&_svg]:size-4 [&_svg]:shrink-0 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50",
        className
      )}
      onClick={(event) => {
        onClick?.(event);
...
function SidebarRail({
  ...
  const { state, setOpen, sidebarWidth, setSidebarWidth, toggleSidebar } = useSidebar();
  ...
  const finishResize = React.useCallback(() => {
    ...
    if (activeResize.didDrag) {
      setSidebarWidth(snapSidebarWidth(activeResize.currentWidth));
    }
  ...
  const handlePointerMove = React.useCallback(
    (event: React.PointerEvent<HTMLButtonElement>) => {
      ...
      const signedDelta =
        ...
        (activeResize.side === "left" ? 1 : -1);
      const nextWidth = clampSidebarWidth(activeResize.startWidth + signedDelta / rootFontSize);

      if (Math.abs(nextWidth - activeResize.startWidth) >= 0.2) {
        activeResize.didDrag = true;
    ...
    [onPointerMove, setOpen, setSidebarWidth, state]
  );
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 after:absolute after:inset-y-0 after:start-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=left]:-end-4 group-data-[side=right]:start-0 sm:flex",
...
function SidebarMenuSub({
  ...
  return (
    <ul
      ...
      className={cn(
        "ms-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-s border-sidebar-border ps-2.5 py-0.5",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
