...
1. **Decide and record.** Open an issue titled `migrate: <package> <from> → <to>` containing the
   ...
   affected: which apps and packages, and which user-visible behavior. The user decides *now*,
   *hold* (add a `HOLD:` rule linking the issue), or *never* (close the bot PR and the issue with the
   reason).
