...
- **Rewrite published or preserved branch history** - Use ordinary new commits. Never reset, rebase, force-push, move preserved refs, or replace a tree to bypass a validation refusal. Stop and escalate if history safety cannot be proved. An active no-mistakes run owns its fixes and publication; do not publish around it.
...
| Working on Renovate, dependency-update automation, or the `RENOVATE_TOKEN` secret | `docs/dependency-updates.md`; drain the queue with the `update-deps` skill (`.agents/skills/`) |
| Migrating a dependency major, or changing the Node/Bun runtime baseline | `docs/dependency-migrations.md` and the `assess-upgrade` skill; `bun run check:runtime-baseline`; decisions go in `docs/dependency-log.md` |
